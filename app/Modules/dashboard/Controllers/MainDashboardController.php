...
class MainDashboardController extends BaseController
{
    ...
    public function getAdministratorDetails(){
    	...
    	$assignments_user = DB::table('assignment')
					...
					->get();
					// dd($assignments_user);
		$assessment=Assessment::take(8)->get();
		//close mahesh
		//soma sekhar
		...
		//eswar
			...
			$list_lession=DB::table('lesson')->orderby('updated_at','desc')->take(8)->lists('name','id')
		     ;//->get();
			// dd($list_lession);
   ...
   // dd('dgcg');
		...
		if(getRole()!= 4)
		{
			...
			$assignments_user = $assignments_user->orderBy('startdatetime', 'desc')
					->take(8)
					->get();
		$assessment=Assessment::where('institution_id','=',$ins)->take(8)->get();
		//close mahesh
		//soma sekhar
	//dd($assessment);
	$class_students = AssignmentUser::join('user_assignment_result', 'user_assignment_result.assignment_id', '=', 'assignment_user.assignment_id')
	...
	//close soma sekhar
		//eswar
			$tech=DB::table('roles')->where('name', 'teacher')->first();
			$stu=DB::table('roles')->where('name', 'student')->first();
			$list_lession=DB::table('lesson')->where('institution_id','=',$ins)->take(8)->lists('name','id');//->
			// dd($tech);
		$list_details=Question::join('question_type','questions.question_type_id','=','question_type.id')
...
public function AdminPopup($ca =0,$id = '10'){
	  ...
	  if(getRole()=="admin" || getRole()=="teacher") {
	   ...
	   //dd($students);
        ...
        if($ca == 'ca'){
// dd($ca);
	    return view('dashboard::dashboard.classaverage',compact('class_students','user','assignments_user','assessment','list_details','slist','tlist','assignments','mark','All_users','complete_users','student_whole','score','user','list_lession','students','stud','tech'));
          }
        elseif ($ca == 'th') {
	
	     return view('dashboard::dashboard.testhistory',compact('class_students','user','assignments_user','assessment','list_details','slist','tlist','assignments','mark','All_users','complete_users','student_whole','score','user','list_lession','students','stud','tech'));
             }
         elseif ($ca == 'wc') {
// dd($ca);
	      return view('dashboard::dashboard.wholeclass',compact('class_students','user','assignments_user','assessment','list_details','slist','tlist','assignments','mark','All_users','complete_users','student_whole','score','user','list_lession','students','stud','tech'));
	       }
	  else{}
	     }
