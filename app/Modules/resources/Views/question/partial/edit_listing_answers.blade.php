...
@foreach($oldAnswers as $key => $answer)
    ...
    <div class='mb18 mr10 mt20 pos_rel'>
        <div class='col-md-2'>
        <label class='question_answer_count'>Answer #{{$ans_count}}<i>*</i></label>
        ...
        <!-- <i class='{{ ($answer["is_correct"] == "YES") ? "switch_on" : "switch_off" }} icons L0 correct' data-answer_selection=''></i> -->
            <input id="input-1" class="ans-chk" value="{{$ans_count}}" type="checkbox" {{ ($answer["is_correct"] == "YES") ? "checked" : "" }} data-group-cls='btn-group-sm'  offLabel="<span class='glyphicon glyphicon-remove'>" onLabel="<span class='glyphicon glyphicon-ok'>">
        <input type='hidden' name='is_correct[]' id='is_correct[]' value='{{ ($answer["is_correct"] == "YES") ? "true" : "false" }}'/>
        </div>
        <div class='col-md-10'>
        <p style='w93 fltL'>
...
@else
    ...
    <div class='mb18 mr10 mt20 pos_rel'>
        <div class='col-md-2'>
        <label class='question_answer_count'>Answer <i>*</i></label>
        <input type="hidden" name="answerIds[]" class='hanswerId' value="{{ $answer['id'] }}">
        <!-- <i class='{{ ($answer["is_correct"] == "YES") ? "switch_on" : "switch_off" }} icons L0 correct' data-answer_selection=''></i> -->
            <!-- <input id="input-1" class="ans-chk" value="{{$ans_count}}" type="checkbox" {{ ($answer["is_correct"] == "YES") ? "checked" : "" }} data-group-cls='btn-group-sm'  offLabel="<span class='glyphicon glyphicon-remove'>" onLabel="<span class='glyphicon glyphicon-ok'>"> -->
